...
- Single Go binary; env config: `PORT`, `RESPOND_API_TOKEN`, `AGENT_CMD`, `DATA_DIR`, `TYPING_DELAY_FACTOR`, `OUTGOING_COMMAND` (optional).
...
- Reply delivery: split the streamed reply on `\n\n`; send each paragraph as a separate respond.io message, each delayed proportionally to its length by `TYPING_DELAY_FACTOR`.
- Optional **New Outgoing Message** webhook: skip messages sent by the agent-server itself (echo filter); for others, prompt the harness with `OUTGOING_COMMAND` followed by the message content.
