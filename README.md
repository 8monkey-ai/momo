...
# Records the messages of a conversation a human answers in the agent's session.
# Optional: leave the block out and momo answers every message itself. Both
# commands are required when the block is there.
session_history:
...
channels:
  respondio:
    ...
    # assigned to it and the ones assigned to nobody, and records the messages of
    # a conversation another assignee owns. 0 leaves every conversation to momo.
