...
- Per-turn harness termination is server-driven only because pi-acp can't survive its pi child dying (silent empty `end_turn` on the next prompt — https://github.com/svkozak/pi-acp/issues/82). Once fixed upstream, revisit harness-side session end (see the FUTURE note in `runTurn` in actor.go).
