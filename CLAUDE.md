...
- **All harness termination is graceful SIGTERM to the whole process group** (already implemented): pi's rpc-mode SIGTERM handler runs its shutdown, persisting the session and firing `session_shutdown{reason:"quit"}` hooks. The process group is signaled (not pi-acp alone) so pi still gets SIGTERM even if pi-acp is wedged. Session gzip-at-rest is currently **de-prioritized**: the pi-session-gzip extension is removed from the gato config until its restore story is settled (see its PR #2 — stub `.jsonl` + a `pi-gz` shim via `PI_ACP_PI_COMMAND`, which pi-acp honors when spawning pi). If it comes back, that shim setup is what makes `session/load` see full history; the server needs no changes either way.
