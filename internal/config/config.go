...
// Config is the configuration momo runs with. The channel blocks, the agent
// block and the extension block stay undecoded so that each of them owns its own
// settings.
type Config struct {
	Listen            string
	MaxConnections    int
	...
	// SessionHistorySync is nil when the file has no such block, which disables the
	// extension.
...
func parse(raw []byte) (*Config, error) {
	...
	cfg := &Config{
		...
		SessionHistorySync: optionalDecoderFor(f.SessionHistorySync),
	}
	if cfg.Listen == "" {
		cfg.Listen = ":8080"
	}
...
// optionalDecoderFor answers no decoder for an absent block. A present block,
// even an empty one, reaches the extension, so the extension refuses settings it
// needs and does not have.
func optionalDecoderFor(node yaml.Node) channel.Decoder {
	if node.IsZero() {
		return nil
	}
	return decoderFor(node)
}
