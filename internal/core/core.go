...
// TextOf is Text's inverse: the text a channel that carries plain text only can
// send for a message's blocks. Blocks that carry no text contribute nothing.
...
// the incoming one can be answered.
type Handler interface {
	Received(ctx context.Context, m Message, reply Reply)
	Sent(ctx context.Context, m Message)
}

// EchoHandler reports every message on the log and answers an incoming one with
// its own content. It proves the reply path end to end and is replaced by the
// agent harness.
