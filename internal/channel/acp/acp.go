...
// with every request authenticated by a bearer token. An ACP client speaks for
// itself, so nothing takes a conversation over and no message is recorded.
func New(lifetime context.Context, decode channel.Decoder, h core.Handler, _ core.Recorder) (channel.Channel, error) {
