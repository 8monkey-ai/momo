...
type notification struct {
	...
	ID     *int                      `json:"id"`
	Params acpv1.SessionNotification `json:"params"`
}
...
func (h *harness) prompt(t *testing.T, connID, sessionID, blocks string) {
	...
	status(t, h.do(t, request{
		body:   rpc(3, acpv1.MethodPrompt, `{"sessionId":"`+sessionID+`","prompt":[`+blocks+`]}`),
...
func (h *harness) createSession(t *testing.T, connID string) string {
	...
	status(t, h.do(t, request{body: rpc(2, acpv1.MethodNewSession, "{}"), connID: connID}), http.StatusAccepted)
...
func TestAPromptWhoseStreamIsGoneStillCompletes(t *testing.T) {
	...
	// Nothing listens to the session's stream, so neither the reply nor the answer
	// to the prompt can be delivered. The POST is still answered, and the endpoint
	// stays usable.
