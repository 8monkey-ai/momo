...
// failing stands for a turn that could not be completed.
type failing struct{}

func (failing) Received(context.Context, core.Message, core.Reply) error {
	return errors.New("the harness stopped during the turn")
}
func (failing) Sent(context.Context, core.Message) {}
...
func (h *harness) initialize(t *testing.T) string {
	t.Helper()
	resp := h.do(t, request{body: rpc(1, acpv1.MethodInitialize, `{"protocolVersion":1,"clientCapabilities":{}}`)})
...
func (h *harness) session(t *testing.T) (connID, sessionID string, connStream, sessionStream *sse) {
	...
	status(t, h.do(t, request{body: rpc(2, acpv1.MethodNewSession,
	...
	var created acpv1.NewSessionResult
	...
	for _, method := range []string{http.MethodPost, http.MethodGet, http.MethodDelete} {
		for _, tc := range []struct {
			...
			t.Run(method+"/"+tc.name, func(t *testing.T) {
				...
				req := tc.req
				req.method = method
				req.body = rpc(1, acpv1.MethodInitialize, "{}")
...
func TestTokenSchemeIsCaseInsensitive(t *testing.T) {
	...
	r, err := http.NewRequest(http.MethodPost, h.url,
		strings.NewReader(rpc(1, acpv1.MethodInitialize, "{}")))
	if err != nil {
		t.Fatal(err)
	}
...
func TestContentNegotiationRejections(t *testing.T) {
	...
	t.Run("POST without json content type", func(t *testing.T) {
		status(t, h.do(t, request{contentType: "text/plain", body: rpc(1, acpv1.MethodInitialize, "{}")}),
...
func TestIdentityHeaderRejections(t *testing.T) {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			req:  request{body: rpc(3, acpv1.MethodNewSession, "{}")},
			want: http.StatusBadRequest,
		},
		{
			name: "POST with an unknown connection id",
			req:  request{body: rpc(3, acpv1.MethodNewSession, "{}"), connID: "0123456789abcdef"},
			want: http.StatusNotFound,
		},
		{
			name: "session-scoped POST without a session id",
			req:  request{body: rpc(3, acpv1.MethodPrompt, `{"prompt":[{"type":"text","text":"hi"}]}`), connID: connID},
			want: http.StatusBadRequest,
		},
		{
			name: "session-scoped POST with a session of another connection",
			req: request{body: rpc(3, acpv1.MethodPrompt, `{"prompt":[{"type":"text","text":"hi"}]}`),
...
func TestBatchIsRefused(t *testing.T) {
	h := newHarness(t)
	resp := h.do(t, request{body: `[` + rpc(1, acpv1.MethodInitialize, "{}") + `]`})
...
func TestMethodsThatNeedAnIdRefuseANotification(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{method: acpv1.MethodInitialize},
		{method: acpv1.MethodNewSession, req: request{connID: connID}},
		{method: acpv1.MethodPrompt, req: request{connID: connID, sessionID: sessionID}},
	} {
	...
	t.Run("session/cancel is accepted without an id", func(t *testing.T) {
		...
		status(t, h.do(t, request{
			body:   `{"jsonrpc":"2.0","method":"` + acpv1.MethodCancel + `","params":{"sessionId":"` + cancelSession + `"}}`,
...
func TestPromptReachesTheCoreWithItsBlocksAsSent(t *testing.T) {
	...
	status(t, h.do(t, request{body: rpc(3, acpv1.MethodPrompt, prompt), connID: connID, sessionID: sessionID}),
		http.StatusAccepted)
	...
	var completed acpv1.PromptResult
...
func TestPromptWithoutUsableContentIsInvalidParams(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			status(t, h.do(t, request{body: rpc(3, acpv1.MethodPrompt, tc.params),
...
func TestUnimplementedMethodLeavesTheConnectionUsable(t *testing.T) {
	...
	status(t, h.do(t, request{body: rpc(4, acpv1.MethodPrompt, `{"prompt":[{"type":"text","text":"still here"}]}`),
		connID: connID, sessionID: sessionID}), http.StatusAccepted)
	var completed acpv1.PromptResult
	unmarshalResult(t, sessionStream.next(t), &completed)
}
...
func TestDeleteReleasesSessionsAndClosesStreams(t *testing.T) {
	...
	status(t, h.do(t, request{body: rpc(3, acpv1.MethodPrompt, `{"prompt":[{"type":"text","text":"hi"}]}`),
		connID: connID, sessionID: sessionID}), http.StatusNotFound)
}

func TestFailedTurnIsAnsweredWithAJsonRpcError(t *testing.T) {
	h := unserved()
	h.serve(t, failing{})
	connID, sessionID, connStream, sessionStream := h.session(t)

	status(t, h.do(t, request{
		body:   rpc(3, acpv1.MethodPrompt, `{"sessionId":"`+sessionID+`","prompt":[{"type":"text","text":"hi"}]}`),
		connID: connID, sessionID: sessionID,
	}), http.StatusAccepted)

	resp := sessionStream.next(t)
	if resp.Error == nil {
		t.Fatalf("the prompt was answered with %s, want a JSON-RPC error", *resp.Result)
	}
	if resp.Error.Code != jsonrpc2.CodeInternalError {
		t.Errorf("code = %d, want %d", resp.Error.Code, jsonrpc2.CodeInternalError)
	}
	if resp.Result != nil {
		t.Errorf("the response carried the result %s, want no stop reason", *resp.Result)
	}
	connStream.silent(t)
}
