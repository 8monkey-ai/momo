...
// echo answers a prompt with the content it carried, so a test sees the reply
...
// failed is a handler whose turn produced no reply.
type failed struct{}

func (failed) Received(context.Context, core.Message, core.Reply) error {
	return errors.New("the harness stopped during the turn")
}

func (failed) Sent(context.Context, core.Message) {}
