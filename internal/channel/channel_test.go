...
func stub(name string) Factory {
	return func(context.Context, Decoder, core.Handler, Recorder) (Channel, error) {
...
func TestBuildReportsWhichChannelFailed(t *testing.T) {
	...
	Register("stub-broken", func(context.Context, Decoder, core.Handler, Recorder) (Channel, error) { return nil, broken })
...
func deliver(conversation string, failed *error) Factory {
	return func(_ context.Context, _ Decoder, h core.Handler, _ Recorder) (Channel, error) {
...
// handover records into the handler it was bound to, the way the session history
// extension does.
type handover struct{ h core.Handler }

func (o handover) RecordUser(ctx context.Context, m core.Message) {
	_ = o.h.Received(ctx, m, func(context.Context, []core.ContentBlock) error { return nil })
}

func (o handover) RecordAssistant(ctx context.Context, m core.Message) {
	o.RecordUser(ctx, m)
}

func recordsOnBuild(conversation string, got *Recorder) Factory {
	return func(_ context.Context, _ Decoder, _ core.Handler, rec Recorder) (Channel, error) {
		*got = rec
		if rec != nil {
			rec.RecordUser(context.Background(), core.Message{Conversation: conversation, Content: core.Text("hi")})
		}
		return fixed{}, nil
	}
}

// TestARecordNamesTheSameConversationAsATurn pins that a record and a turn queue
// on one conversation: the harness holds a conversation by its name, so a record
// that named the contact alone would run beside that contact's turn.
func TestARecordNamesTheSameConversationAsATurn(t *testing.T) {
	isolateFactories(t)
	var rec Recorder
	Register("respondio", recordsOnBuild("123", &rec))
	got := &recorder{}

	build := func(h core.Handler) Recorder { return handover{h} }
	if _, err := Build(context.Background(), configured("respondio"), got, build); err != nil {
		t.Fatalf("Build: %v", err)
	}
	if len(got.received) != 1 || got.received[0] != "respondio:123" {
		t.Fatalf("received = %v, want [respondio:123]", got.received)
	}
}

func TestAChannelGetsNoRecorderWithoutSessionHistorySync(t *testing.T) {
	isolateFactories(t)
	var rec Recorder
	Register("respondio", recordsOnBuild("123", &rec))
	...
	if rec != nil {
		t.Fatalf("the channel got the recorder %+v, want none", rec)
	}
}
