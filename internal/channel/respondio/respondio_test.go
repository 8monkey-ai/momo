...
// outgoing is the payload of a sent message from the named sender source. An
// empty source leaves the sender block out.
...
// TestAMomoUserIDRequiresTheHistoryExtension pins that momo refuses the
// configuration rather than losing the messages of another assignee in silence.
...
// replyCapture answers the reply its handler was given.
...
// TestARecordedMessageCannotAnswerTheContact pins that a record is given no
// reply: the operator owns the conversation, so momo must not be able to write
// into it.
