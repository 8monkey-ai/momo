...
type settings struct {
	...
	// MomoUserID is the respond.io user momo is in the workspace. Zero leaves
	// every conversation to momo.
...
func New(_ context.Context, decode channel.Decoder, h core.Handler, history core.Handler) (channel.Channel, error) {
	...
	// Naming momo in the workspace hands conversations to humans, and only the
	// extension keeps the messages momo does not answer.
...
// received keeps the message in the agent's session when another assignee owns
// the conversation, so the session a later turn resumes holds what was said
// meanwhile.
func (h *webhook) received(ctx context.Context, ev event, m core.Message, contactID string) {
	if !h.momoOwns(ev) {
		// The operator answers, so a record gets no reply and reports its failure to
		// the log alone.
