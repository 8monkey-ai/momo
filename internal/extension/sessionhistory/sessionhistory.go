...
//
// The extension is optional: a deployment without human handover runs without
// it. Which message is recorded, and which is answered, is the channel's
// decision; this package owns the command a record is written with and nothing
// else.
...
// record writes one message into the session as a turn of its conversation: the
// agent's own prompt path, and the same lock every other turn of that
// conversation waits for.
