...
// Turn runs one turn of one conversation.
...
// Prompt is the whole operation a turn is, and the port an extension prompts
// through, so a record and a turn of one conversation share the lock and the
// session and never run at the same time. The timeout bounds everything the
// prompt does, the wait for the conversation included, so a message behind a
// stopped prompt is released as well.
...
// acquire holds the conversation for one prompt. Each conversation has a channel of
// capacity one, so prompts of different conversations never wait for each other.
//
